...
export class CommercesService {
  ...
  async getBTCMAPData() {
    ...
    const dataFiltered = data.filter((item) => {
      if (item['id'].toString().includes('node:')) {
        if (item['osm_json']['lat'] > 90 || item['osm_json']['lat'] < -90)
          return false;
        if (item['osm_json']['lon'] > 180 || item['osm_json']['long'] < -180)
          return false;
        return true;
      }
      return false;
  ...
  async getCommercesVersion() {
    try {
      ...
      if (error instanceof Error)
        this.logger.error(`Error getting commerce version: ${error.message}`);
  ...
  async updateCacheCommerces() {
    const allCommerces = await this.commerceModel.find(
      {},
      { 'osm_json.tagsString': 0 },
    );
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new BadRequestException(
          `Failed to get nearest commerces: ${error.message}`,
        );
      }
