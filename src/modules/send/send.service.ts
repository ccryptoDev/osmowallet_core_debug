...
export class SendService {
  ...
  async refundAutoConverToSendInvoiceTransaction(data: RefundSendDto, coin: Coin) {
    ...
    await this.coinRepository.manager.transaction('SERIALIZABLE', async entityManager => {
      ...
      if (!data.createSendTransaction.amounts.osmoFiatFeeToCredit || !data.createSendTransaction.amounts.totalUserFiatToDebit) throw new BadRequestException('Amount not found')
  ...
  async estimateSend(data: EstimateBtcSendDto) {
    ...
    if (isLnInvoice) {
      ...
      if (amountSats === undefined && data.amountSats && data.amountSats <= 0) throw new BadRequestException('amount debe ser mayor que 0');
