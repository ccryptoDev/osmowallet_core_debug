...
export class NormalSendFiat implements SendFiat {
    ...
    async sendFiat(): Promise<boolean> {
        ...
        if (!fromUser) throw new BadRequestException('Invalid user')
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            ...
            if (!userWallet) throw new BadRequestException('Invalid user wallet')
            if (!receiverWallet) throw new BadRequestException('Invalid receiver wallet')
