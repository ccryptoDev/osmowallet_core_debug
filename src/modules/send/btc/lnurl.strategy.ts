...
export class Lnurl extends Send implements SendBtc {
    ...
    async sendNative(data: SendBtcData): Promise<any> {
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            const userBtcWallet = await findAndLockWallet({ entityManager: entityManager, coinId: data.payload.coinId, userId: data.authUser.sub })
    ...
    async sendAutoconvert(data: SendBtcData): Promise<any> {
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            ...
            if (new Decimal(userFiatWallet.availableBalance).lessThan(new Decimal(fiats))) throw new BadRequestException('Insufficient balance',);
    ...
    async createAutoconvertTransactions(data: CreateTransaction): Promise<void> {
        try {
            ...
            const lnurlResponse = await this.ibexService.payLnURL(paramsToSend, data.payload.amount * 1000, process.env.IBEX_NATIVE_OSMO_ACCOUNT_ID ?? '');
