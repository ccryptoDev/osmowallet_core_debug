...
export class MeService {
  ...
  async updateUsername(authUser: AuthUser, data: UpdateUsernameDto) {
    try {
      ...
      if (!user) throw new BadRequestException('No se encontro ningun usuario con este id');
      ...
      if (!ibexAccount) throw new BadRequestException('No se encontro ningun ibex account con este id');
  ...
  async getProfile(authUser: AuthUser) {
    ...
    if (!profile) throw new BadRequestException('No se encontro ningun perfil de usuario con este id');
    return profile;
  }
  ...
  async createBankAccount(authUser: AuthUser, data: BankAccountDto) {
    try {
      ...
      const user = await this.userRepository.findOneBy({ id: authUser.sub });
      if (!user) {
        throw new BadRequestException('No se encontro ningun usuario con este id');
      }
  ...
  async updatePreference(authUser: AuthUser, data: PreferenceDto) {
    ...
    if (!preferenceRecord)
      throw new BadRequestException('No se encontro ninguna preferencia');
