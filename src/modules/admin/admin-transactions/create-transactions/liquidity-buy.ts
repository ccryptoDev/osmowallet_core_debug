...
export class LiquidyBuy extends LiquidityTransaction implements AdminTransaction {
    ...
    async validateData(): Promise<void> {
        ...
        if (coin!.acronym == CoinEnum.SATS || coin!.acronym == CoinEnum.USDT) {
            throw new BadRequestException(`Invalid coin: ${coin!.acronym}`);
        }
    }
    ...
    async create(): Promise<void> {
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            ...
            await this.createAndInsertTransaction(wallet!,this.data.fiat.amount,wallet!.availableBalance, TransactionSubtype.DEBIT_FIAT_BUY,transactionGroup.id,entityManager)
            const cryptoSubtype: TransactionSubtype = cryptoWallet!.coin.acronym == CoinEnum.SATS ? TransactionSubtype.CREDIT_BTC_BUY : TransactionSubtype.CREDIT_STABLE_OSMO
            await this.createAndInsertTransaction(cryptoWallet!,amountCryptoToCredit.toNumber(),cryptoWallet!.availableBalance, cryptoSubtype,transactionGroup.id,entityManager)
