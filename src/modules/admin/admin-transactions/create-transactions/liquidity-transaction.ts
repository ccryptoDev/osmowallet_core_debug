...
export abstract class LiquidityTransaction {
    ...
    async createAndInsertTransaction(wallet: Wallet, amount: number, balance: number, subtype: TransactionSubtype, transactionGroupId: string, entityManager: EntityManager): Promise<void> {
    ...
    async createAndInsertTransactionFee(amount: number, coinId: string, transactionGroupId: string, entityManager: EntityManager): Promise<void> {
