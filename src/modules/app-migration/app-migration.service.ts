...
export class AppMigrationService {
  ...
  async storeBPT() {
    try {
      ...
      for (let i = 0; i < sheets.length; i++) {
        const temp = xlsx.utils.sheet_to_json<string>(file.Sheets[file.SheetNames[i] as any] ?? {});
        temp.forEach((res) => {
          data.push(res);
        });
      }
  ...
  async getFileBytes(data: any): Promise<string | null> {
    ...
    try {
      ...
      return this.googleCloudStorageService.getPublicUrl('osmo-bpts', path);
    } catch (error) {
      return null;
    }
  }
  ...
  private async getUserBalances(
    ...
    try {
      ...
      if (!userBalances) throw new Error('user balance found')

      const usdBalance = userBalances.currencies.find(currency => currency.currency.code === 'USD')
      if (!usdBalance) throw new Error('usd balance found')

      const gtqBalance = userBalances.currencies.find(currency => currency.currency.code === 'GTQ')
      if (!gtqBalance) throw new Error('gtq balance found')
      ...
      return {
        gtqBalance: gtqBalance.available_balance / 100,
        usdBalance: usdBalance.available_balance / 100,
  ...
  async getRecurrentBuys(
    ...
    if (recurrentBuy.docs.length > 0) {
      const data = recurrentBuy.docs.map((doc) => {
  ...
  async getPreferences(rehiveUser: RehiveUser): Promise<RehiveUserPreference | undefined> {
    ...
    if (preferencesData !== undefined) {
      return {
        fiatCoin: preferencesData['strCoin'],
        cryptoCoin: preferencesData['strCriptoCoin'],
      };
    }
  }
  ...
  async creteUserData(
    ...
    try {
      await this.userRepository.manager.transaction(async entityManager => {
        ...
        const fiatCoinPreference = FiatCoinPreference[fiatCoinString as keyof typeof FiatCoinPreference];
        const cryptoCoinPreference = CryptoCoinPreference[cryptoCoinString as keyof typeof CryptoCoinPreference];
        ...
        const ibexAccount = entityManager.create(IbexAccount, {
          ...
          user: user,
          usernameId: customLNUsername ? customLNUsername.id : undefined
        });
  ...
  async createVerification(verification: any, user: User) {
    try {
      if (verification && verification.verification_id != null) {
        let status: Status = Status.CREATED;
  ...
  async getUserDataMigration(rehiveUser: RehiveUser | undefined) {
    if (rehiveUser === undefined) return;
  ...
  async updateDocumentNumber() {
    const kycVerifications = await this.kycVerificationRepository.find({
      where: {
  ...
  async runMigration() {
    ...
    let usersFixed = this.users.map((user: any) => {
