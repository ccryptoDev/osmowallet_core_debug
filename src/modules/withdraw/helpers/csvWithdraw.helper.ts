...
export class CsvWithdrawHelper {
    ...
    private achElement(transaction: Transaction, osmoBankAccounts: Array<OsmoBankAccount>, index: number) {
        const osmoBankAccount = osmoBankAccounts.find(account => account.coin.acronym == transaction.transactionGroup.metadata['bankAddress' as keyof typeof transaction.transactionGroup.metadata]['currency'])
        ...
        return {
            ...
            accountNumber: transaction.transactionGroup.metadata['bankAddress' as keyof typeof transaction.transactionGroup.metadata]['accountNumber'],
            accountType: (bankType == BankAccountType.AHORROS || bankType == 'SAVINGS') ? 4 : 3,
            referenceNumber: index,
            accountName: transaction.transactionGroup.metadata['bankAddress' as keyof typeof transaction.transactionGroup.metadata]['accountHolder'],
            bankCode: transaction.transactionGroup.metadata['bankAddress' as keyof typeof transaction.transactionGroup.metadata]['bankCode'],
        }
    }
    ...
    async createCsv(withdraws: Array<Transaction>, osmoBanksAccounts: Array<OsmoBankAccount>, bankAccounts: Array<BankAccount>): Promise<SendgridTemplate> {
        ...
        withdraws = withdraws.filter(withdraw => withdraw.transactionGroup.metadata['bankAddress' as keyof typeof withdraw.transactionGroup.metadata]['bankCode'] !== 0)
        let i = 0
