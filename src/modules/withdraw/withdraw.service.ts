...
export class WithdrawService {
    ...
    async getWithdrawMethods(authUser: AuthUser) {
        ...
        const withdrawMethodsModified = withdrawMethods.map((method: PartialBy<WithdrawalMethod, "availableCoins"> & { coins?: Coin[] }) => {
    ...
    async generateBankWithdrawReport() {
        ...
        if (currentHourInGT === 8 || currentHourInGT === 17) {
            const withdraws = await this.transactionRepository.find({
                relations: { transactionGroup: true, wallet: { account: { user: true } } }, where: {
    ...
    async withdraw(authUser: AuthUser, body: WithdrawDto) {
        ...
        switch (withdrawMethod.name) {
            ...
            case WithdrawalMethodEnum.SINPE: {
                strategy = new SinpeWithdraw(body, this.userRepository.manager, user, this.googleTaskService, this.ridiviService, this.pushNotificationService, withdrawMethod)
                break;
            }
            case WithdrawalMethodEnum.SINPE_MOBILE: {
                strategy = new SinpeMobileWithdraw(body, this.userRepository.manager, user, this.ridiviService, this.pushNotificationService, withdrawMethod, this.googleTaskService)
                break;
            }
