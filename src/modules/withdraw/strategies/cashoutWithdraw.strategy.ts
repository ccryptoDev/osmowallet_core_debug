...
export class CashoutWithdraw implements Withdraw {
    ...
    async withdraw() {
        ...
        if (!amountSats) throw new BadRequestException('Invalid address')
        ...
        if (!gtqCoin) throw new BadRequestException('Coin not found')
    ...
    async createTransaction(data: CashOutPayload) {
        ...
        if (!ibexAccount) {
            throw new BadRequestException('Ibex account not found');
        }
        await this.ibexService.doCashout(data.payload.address, ibexAccount.account, data.amounts.totalUserBtcToDebit ?? 0)
        const invoice = await this.ibexService.getInvoiceFromBolt11(data.payload.address.toLowerCase())
        let osmoBusiness: OsmoBusinessBpt | undefined
        if (invoice) {
            osmoBusiness = await this.manager.findOneBy(OsmoBusinessBpt, { bptName: invoice.memo }) ?? undefined
        }
