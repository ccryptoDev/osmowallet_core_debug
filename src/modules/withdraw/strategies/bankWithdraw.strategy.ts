...
export class BankWithdraw implements Withdraw {
    ...
    async withdraw(): Promise<any> {
        ...
        if (!user || !feature || !fundingMethod) throw new NotFoundException('User, feature or funding method not found')
        ...
        if (!records) throw new NotFoundException('User transaction limit not found')
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            ...
            if (!wallet || !osmoWallet || !osmoWalletFee) throw new NotFoundException('Wallets not found')
    ...
    async validInputData() {
        ...
        const coin = await this.manager.findOneBy(Coin, {
            id: this.data.coinId
        })
