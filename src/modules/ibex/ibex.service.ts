...
export class IbexService {
    private BASE_URL = process.env.IBEX_BASE_URL
    ...
    private async getToken(): Promise<string> {
        ...
        if (!ibexAccessToken) { throw new BadRequestException('Ibex access token not found') }
    ...
    async sendOnChain(accountId: string, data: SendDto, transactionGroupId?: string): Promise<PayOnChainResponse | undefined> {
        try {
            const ibexAccessToken = await this.getToken()
            const config = {
                method: 'POST',
    ...
    async getInvoiceFromBolt11(bolt11: string): Promise<IbexInvoice | undefined> {
        try {
            const ibexAccessToken = await this.getToken()
            const config = {
                method: 'GET',
    ...
    async doCashout(bolt11: string, accountId: string, amount: number): Promise<IbexLightningTransaction | undefined> {
        try {
            const ibexAccessToken = await this.getToken()
            const config = {
                method: 'POST',
    ...
    async getParams(lnurlDecoded: string) {
        try {
            ...
            throw new IbexServiceException(error || 'Failed to get params');
        }
    }
    ...
    async createOnchainAddress(accountId: string, currency: CurrencyEnum, returnData: boolean = false): Promise<string | undefined> {
        ...
        if (returnData) return address
            ...
            if (!userAddresses) throw new BadRequestException("User addresses not found")
            ...
            const addr = userAddresses.addresses.find(a => a.type === Types.ONCHAIN)

            if (addr) {
                const addresses = userAddresses.addresses.map(a => {
                    if (a.type === Types.ONCHAIN) {
                        a.address = address
                    }
                    return a
                })
                await this.addressesModel.updateOne({ user: ibexAccount.user.id }, { $set: { addresses } });
            }
        }
    }
    ...
    async createAddresses(accountId: string) {
        ...
        try {
            ...
            for (const coin of coins) {
                ...
                if (!onchainAddress) throw new BadRequestException("Onchain address not found")
