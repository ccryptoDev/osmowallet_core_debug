...
export class StableFunding implements Funding {
    ...
    async fund(): Promise<any> {
        ...
        await this.manager.transaction('SERIALIZABLE', async transactionalEntityManager => {
            ...
            if (!userWallet || !osmoWallet || !osmoFeeWallet) throw new BadRequestException('No se encontraron las billeteras necesarias para realizar la transacción')
    ...
    private async validateInputData() {
        ...
        if (errors.length > 0) {
            const messageError = errors.map(error => Object.values(error)).join(', ');
        ...
        if (!fundingTransactionLimit || !tierUser) throw new BadRequestException('No se encontro el limite de transacción')
