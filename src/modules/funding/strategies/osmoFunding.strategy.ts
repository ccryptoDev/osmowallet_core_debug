...
export class OsmoBankFunding extends DynamicDtoValidator implements Funding {
    ...
    async fund() {
        ...
        await this.manager.transaction('SERIALIZABLE', async entityManager => {
            ...
            if (!userWallet || !osmoWallet) throw new BadRequestException('No se encontraron las billeteras necesarias para realizar la transacción')

            const method: TransactionMethodEnum = TransactionMethodEnum[this.fundingMethod.name as keyof typeof TransactionMethodEnum]
