...
export class AuthService {
  ...
  async verifyMobile(authUser: AuthUser, otp: number) {
    try {
      ...
      if (otpRecord) {
        ...
        if (!verificationRecord) throw new NotFoundException("verificationRecord not found")
  ...
  async verifyPassword(
    ...
    const [salt, storedHash] = storedPassword.split('.') as [string, string];
    const hash = crypto
  ...
  async storeSession(
    ...
    if (!authTokenRecord) throw new BadRequestException('Invalid token');
    if (data.token != null) {
