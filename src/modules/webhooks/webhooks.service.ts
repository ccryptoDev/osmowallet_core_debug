...
export class WebhooksService {
  ...
  async sellAutoconvertToSend(user: User, amountSats: number): Promise<boolean> {
    ...
    if (!autoconvertRecord) return false;
  ...
  async receiveOnChain(data: OnChainTransactionDto, currency: CurrencyEnum) {
    ...
    if (!ibexAcount) throw new BadRequestException('Account Not Found');
  ...
  private async createFullOnChainTransactionReceive(user: User, data: PartialBy<OnChainTransactionDto, "webhookSecret">) {
  ...
  async payingOnchain(
    data: PartialBy<OnChainTransactionDto, "webhookSecret">,
  ) {
    if (data.webhookSecret != process.env.IBEX_WEBHOOK_SECRET) throw new UnauthorizedException();
    const ibexAcount = await this.ibexAccountRepository.findOne({
      relations: {
    ...
    if (!transactionGroup) throw new BadRequestException('Transaction Group Not Found');
  ...
  private async updateOnChainTransactionSend(user: User, data: PartialBy<OnChainTransactionDto, "webhookSecret">) {
    const transactionDetail = await this.transactionDetailRepository.findOne({
    ...
    if (!transactionDetail) throw new BadRequestException('Transaction Detail Not Found');
    ...
    if (transactionGroup.transactionCoin.acronym != CoinEnum.SATS) {
      ...
      if (!transaccion) throw new BadRequestException('Transaction Not Found');
  ...
  async payingInvoice(query: IbexPayingQueryDto, data: PayingInvoiceDto) {
    ...
    if (data.transaction.payment.status.name == IbexStatuses.SUCCEEDED) {
      ...
      this.createFeeRecordLeft(query, data, fromUser);
    }
  }
  ...
  async createFeeRecordLeft(query: IbexPayingQueryDto, data: PayingInvoiceDto, user: User) {
    ...
    if (!networkFee) throw new BadRequestException('Network Fee Not Found');
  ...
  async prepareDataToRefund(query: IbexPayingQueryDto, amount: number) {
    ...
    if (!transactionGroupIdExists) throw new BadRequestException('Transaction Group Not Found');
    const transactionGroup = await this.transactionGroupRepository.findOne({
      relations: {
        fromUser: true,
        transactionCoin: true,
  ...
  async storeLnReceivedMSat(data: LightningInvoiceDto) {
    ...
    if (!ibexAccount) throw new BadRequestException('Account not found');
  ...
  private async storeSats(user: User, amount: number, data: LightningInvoiceDto): Promise<TransactionGroup> {
    ...
    let transactionGroup: TransactionGroup | null = null;
    const coin = await this.coinRepository.findOneBy({ acronym: CoinEnum.SATS })
    if (!coin) throw new BadRequestException('Coin not found');
